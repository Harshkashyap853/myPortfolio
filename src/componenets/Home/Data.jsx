...
function Data() {
    ...
    const wordsList = " Html,  CSS,  Javascript,  React,  C++";
    const words = wordsList.split(', ');

     useEffect(() => {
         const txtElement = document.querySelector('.animated_text');
     ...
     function entry() {
        ...
        function eraseText() {
          ...
          function removeLetter() {
            // Ensure that count is within a valid range
            if (count >= 0 && count < spans.length) {
              spans[count].classList.remove('active');
              count--;
          
              // Check if there are more letters to remove
              if (count === -1) {
                clearInterval(removeInterval);
                setWordsCount(wordsCount + 1);
              }
            } else {
              // If count is out of range, clear the interval to avoid further errors
              clearInterval(removeInterval);
            }
          }
        }
  ...
  return (
    <div className='home__data'>
        <h1 className='home__title'>Harsh Kashyap
        <svg
                  width="36"
                  height="36"
                  viewBox="0 0 48 48"
        ...
        <p className='home__description'>Currently Working on {' '} 
            <span className='animated_text'></span>
        </p>
